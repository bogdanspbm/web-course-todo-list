<?php
require_once  $_SERVER['DOCUMENT_ROOT'] . '/api/postgres/lib/get_task.inc';
require_once  $_SERVER['DOCUMENT_ROOT'] . '/api/libs/string_lib.inc';
...
<div class="container-wrapper">
    <div class="container">
        ...
        <form enctype="multipart/form-data" id="task-create-form" action="/api/postgres/redirect/create_task.php"
              method="POST" class="new-task">
            <div class="new-task-container">
                <div class="horizontal-container">
                    <input readonly onfocusout="document.setInputValue(this, 'title');" value="<?php echo $task['title']; ?>"
                ...
                <textarea readonly style="width: 100%; height: 14px;" name="desc" id="desc" maxLength="200" style="height: 14px"
                    ...
                    <div class="file-container">
                        <div id="upload-file-button" class="upload-file-button"
                             onclick="document.downloadFile('<?php echo $task['uid']; ?>','<?php echo $task['file_name']; ?>');"><?php echo $task['file_name']; ?>
                        </div>
                    </div>
                </div>
                <input readonly name="task-uid" type="hidden" id="task-uid" value="<?php echo $task['uid']; ?>">
            </div>
            <div class="new-task-footer">
                <input readonly value="<?php echo date("Y-m-d", $task['task_date_long']); ?>" name="task-date" id="task-date"
                ...
                <div class="custom-color-picker" style="width: 64px;">
                    <button disabled type="button" name="color-display" id="color-display"
                            style="background: <?php echo $task['color']; ?>">Цвет
                    </button>
                    <input readonly value="<?php echo $task['color']; ?>"
